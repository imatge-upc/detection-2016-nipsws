...
feature_size = 7
size_shallower_feature = 32
size_deeper_feature = 16
...
def obtain_descriptor_from_feature_map(feature_maps, region_coordinates):
    initial_width = region_coordinates[2]
    initial_height = region_coordinates[3]
    scale_aux = math.sqrt(initial_height*initial_width)/math.sqrt(feature_size*feature_size)
    if scale_aux > size_shallower_feature:
        scale = size_shallower_feature
        feature_map = feature_maps[1]
    else:
        scale = size_deeper_feature
    ...
    if roi.shape[1] < feature_size & roi.shape[2] < feature_size:
        features = interpolate_features_from_roi(roi)
